...
function ajaxQuery(url, setup) {
	...
	ajaxHandler = $.ajax({
		url: url,
		type: (setup && 'type' in setup ? setup['type'] : '<?= $e->UiComponentAjax->getConfig("defaultRequestType") ?>'),
		timeout: (setup && 'timeout' in setup ? setup['timeout'] : <?= $e->UiComponentAjax->getConfig("defaultTimeout") ?>),
		async: (setup && 'isAsync' in setup ? setup['isAsync'] : <?= ($e->UiComponentAjax->getConfig("defaultIsAsync") ? "true" : "false") ?>),
		cache: (setup && 'isCache' in setup ? setup['isCache'] : <?= ($e->UiComponentAjax->getConfig("defaultIsCache") ? "true" : "false") ?>),
		crossDomain: (setup && 'isCrossDomain' in setup ? setup['isCrossDomain'] : <?= ($e->UiComponentAjax->getConfig("DefaultIsCrossDomain") ? "true" : "false") ?>),
			...
			<?php
				if ($e->isDevel()) {
					...
					?>
						...
						$('#UiComponentNotice').UiComponentNotice('open', ['<?= $e->UiComponentAjax->getConfig("ajaxErrorText") ?>', 'ajaxResponseError']);
					<?php
				}
			?>
		},
